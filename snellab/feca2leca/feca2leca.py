...
def collect_leca_information(euk_clade, feca_count, leca_dict, representing = None, human_seqs = None, human_represent = None, human_seq_info = None, scrollsaw = True):
    for i, leca in enumerate(euk_clade.iter_search_nodes(identity = 'LECA')):
        ...
        if scrollsaw:
            ...
            leca_dict[leca_id] = (str(leca.support), str(leca.coverage), str(leca.redundancy), ','.join(list(leca_human.keys())), ','.join(list(leca_human.values())), ','.join(seqs), ','.join(repr_seqs))
        else:
        ...
        for child in unknown.get_children():
            ...
            if scrollsaw:
                if 'HSAP' in child.repr_species:
                    ...
                    unknown_human = {}
                unknown_dict[unknown_id] = (str(unknown.support), str(child.coverage), ','.join(unknown_human.keys()), ','.join(unknown_human.values()), ','.join(seqs))
            else:
