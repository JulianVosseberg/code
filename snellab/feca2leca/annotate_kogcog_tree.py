...
# Parse arguments
parser = argparse.ArgumentParser(description = "This script identifies FECA-2-LECA duplications in KOG-COG trees, determines the best prokaryotic outgroup (if any), and performs a branch length analysis.")
parser.add_argument("tree", help = 'tree file (either .treefile or .contree)')
parser.add_argument("-p", metavar = "prefix", help = "prefix for output files (DEFAULT: basename tree file)")
...
#parser.add_argument("-l", metavar = "xx", help = "number of Diphoda species for LECA calling (DEFAULT: 1)", type = int, default = 1)
...
if euk_only:
    ...
    collect_leca_information(tree, 1, leca_dict, representing = None, scrollsaw = False)
    collect_dupl_information(tree, 1, dupl_dict, mode = mode, corresponding = corresponding)
    collect_unknown_information(tree, 1, unknown_dict, representing = None, scrollsaw = False)
    ...
    for euk_clade in feca_clades:
        annotate_non_scrollsaw(euk_clade, duplication_criterion, consistency)
        feca_count += 1
    ...
    ancestry_out.write('Family\tFECA\tFECA support\tLECAs\tUnknowns\tSister1 name\tSister1 support\tSister2 name\tSister2 support\tLCA sister\tAncestry\tMajority\tSupport\n')
    for feca_count, euk_clade in enumerate(feca_clades): # Again, now FECA nodes assigned
        ...
        # Collect information on LECA nodes, duplications (including duplication lengths) and unknown nodes
        collect_leca_information(euk_clade, feca_count, leca_dict, representing = None, scrollsaw = False)
        collect_dupl_information(euk_clade, feca_count, dupl_dict, mode = mode, corresponding = corresponding)
        collect_unknown_information(euk_clade, feca_count, unknown_dict, representing = None, scrollsaw = False)

    ancestry_out.close()
    # Annotate non-FECA clades
    with open(outdir_prefix + '_non_feca.tsv', 'w') as non_feca_out:
        non_feca_out.write('Family\tNon-FECA\tDonor taxon\tAncestry\tSupport\tSpecies\tSequence IDs\n')
    ...
    if feca_count != 0:
        with open(outdir_prefix + '_fecas.tsv', 'w') as fecas_out, open(outdir_prefix + '_branch_lengths.tsv', 'w') as branch_lengths_out:
            fecas_out.write('Family\tFECA\tTrusted\tAncestry\tLECAs\tSister-FECAs\n')
            branch_lengths_out.write('Family\tFECA\tAncestry\tLECAs\tProkaryotic sister branch length\tRaw stem lengths\tStem lengths\tEukaryotic branch lengths\n')
    ...
    with open(outdir_prefix + '_fecas_strict.tsv', 'w') as fecas_strict_out:
        fecas_strict_out.write('Family\tFECA\tTrusted\tAncestry\tLECAs\tContent\n')
        for feca_no in bl_information:
...
print('Family\tFECAs (normal)\tFECAs (strict)\tFECAs (after merging)\tLECAs\tUnknowns\tNon-FECAs')
if euk_only:
...
with open(outdir_prefix + '_lecas.tsv', 'w') as lecas_out:
    lecas_out.write('Family\tFECA\tAncestry\tLECA\tSupport\tSeqs\n')
    for leca, info in leca_dict.items():
        ...
        print(prefix, feca_no, ancestry, leca, '\t'.join(info[:6]), sep = '\t', file = lecas_out)

with open(outdir_prefix + '_duplication_lengths.tsv', 'w') as duplication_lengths_out:
    duplication_lengths_out.write('Family\tFECA\tAncestry\tDuplication\tSupport\tSpecies overlap\tDuplication consistency\tOGs\tRaw duplication lengths\tDuplication lengths\tEukaryotic branch lengths\n')
...
with open(outdir_prefix + '_unknowns.tsv', 'w') as unknowns_out:
    unknowns_out.write('Family\tFECA\tAncestry\tUnknowns\tSupport\tSeqs\n')
