...
with open('/home/julian/julian2/pfam_hmm/improved_pipeline/euk_fasta/' + prefix + '_seqids.list') as all_seqs_file:
...
representing, human_represent = assign_all_seqs(tree_seqs, all_seqs, euk_only, prefix)
...
if euk_only:
    ...
    human_seqs = collect_leca_information(tree, 1, leca_dict, representing, human_seqs, human_represent, human_seq_info)
    collect_dupl_information(tree, 1, dupl_dict, mode = mode, corresponding = corresponding)
    human_seqs = collect_unknown_information(tree, 1, unknown_dict, representing, human_seqs, human_represent, human_seq_info)
    ...
    for feca_count, euk_clade in enumerate(feca_clades): # Again, now FECA nodes assigned
        ...
        # Collect information on LECA nodes, duplications (including duplication lengths) and unknown nodes
        human_seqs = collect_leca_information(euk_clade, feca_count, leca_dict, representing, human_seqs, human_represent, human_seq_info)
        collect_dupl_information(euk_clade, feca_count, dupl_dict, mode = mode, corresponding = corresponding)
        human_seqs = collect_unknown_information(euk_clade, feca_count, unknown_dict, representing, human_seqs, human_represent, human_seq_info)
...
with open(outdir_prefix + '_lecas.tsv', 'w') as lecas_out, open(outdir_prefix + '_lecas_all_seqs.tsv', 'w') as lecas_all_seqs_out:
    ...
    for leca, info in leca_dict.items():
        ...
        print(prefix, leca, info[0], info[5], info[6], sep = '\t', file = lecas_all_seqs_out)
...
with open(outdir_prefix + '_unknowns.tsv', 'w') as unknowns_out:
    unknowns_out.write('Pfam\tFECA\tAncestry\tUnknowns\tSupport\tCoverage\tHuman seqs\tHuman name\tSeqs\n')
