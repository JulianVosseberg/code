...
# State usage
function usage() {
    echo -e "Usage: \n\tdoStandardIQTREE.sh -a <alignment_file> [ -p <prefix> ] [ -m <MF+tree|MF|tree(provide model)> ] [ -s <substitution model> ] [ -t <nuclear|mitochondrial|chloroplast|viral> ] [ -c <threads> ] [ -f ] [ -x ] [ -r ]\n"
...
if $finder; then
    ...
    if ! $fast; then
       if [[ "$model" = *"C"[1-6]"0" ]]; then
	    ...
	    iqtree -s $aln -pre ${prefix}_MF_$model -mset $model -nt $threads -m MF $mtree -quiet
...
if $tree; then
	...
	iqtree -s $aln -m $model -pre $prefix -nt $threads -alrt 1000 -bb 1000 -wbtl -quiet
    fi
    echo "Done!"
fi
