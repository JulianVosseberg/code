        ...
        for index, hit in enumerate(seq_pfam[seq]):
            ...
            if overlap <= -1 * overlap_cutoff: # Overlapping
                if i_value < hit[1]: # Check which hit is better
                    seq_pfam[seq][index] = [pfam, i_value, seq_start, seq_stop, hmm_start, hmm_stop]
                break
            ...
            seq_pfam[seq].append([pfam, i_value, seq_start, seq_stop, hmm_start, hmm_stop])
    except KeyError:
        seq_pfam[seq] = [[pfam, i_value, seq_start, seq_stop, hmm_start, hmm_stop]]
hmmfile.close()

pfam_seqs = {}
for seq, hits in seq_pfam.items():
