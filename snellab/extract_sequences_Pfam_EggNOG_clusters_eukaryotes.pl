...
## Original script by Jolien, updated for Pfam and Eukarya4 dataset by Julian
## This script searches for the eukaryotic sequences of the Pfam families that are within a cluster;
## As inputs, it uses the file containing the (numbered) clusters, the file containing the sequence members of each Pfam, and the fasta file containing eukaryotic EggNOG sequences. 
...
my $cluster_contents="BBH_BH_clusters.eukaryotic_Pfam_sequences.tab";
my $cluster_fasta_directory="BBH_BH_clusters.eukaryotic_Pfam_sequences_fastas";
...
open (FAMILY, "<".$pfam_families) or die "Cannot open Pfam families file\n";
while(<FAMILY>){
	my $line=$_;
	chomp $line;
	my @data = split("\t", $line);
