...
##Made by Joran, updated with meta mode by Julian
...
# state usage
function usage() {
    echo "Usage: uppmax_spades.sh -f <fw.fastq.gz> -r <rv.fastq.gz> -s <singles.fastq.gz> -t <threads> -o <output_dir> -l <memory_limit_in_GB> -k <kmer1,kmer2,kmer3,etc> -m <hammer|assembly|standard|meta|continue>"
    exit
}

# if number of arguments is less than 16, invoke usage function
if [ "$#" -lt "16" ]; then
    usage
fi
...
# load tools
module load bioinfo-tools spades/3.7.0
...
# run metaSPAdes
if [ "$mode" = "meta" ]; then
    spades.py --meta -1 $fwdReads -2 $rvrReads -s $unpReads -t $threads -o $outpdir -m $memlimit -k $kmers
fi
